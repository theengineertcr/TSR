    ...
    float MoraleModifier)
{
    ...
    if ( HasProtection( 'IProtectFromSting' ) )
    {        
        return;
    }
	...
	if (IsConscious())
	{
		...
		{
			// If the AI is hit by the less lethal shotgun, greade will be None.
			GrenadeLocation = Location;
			Distance = 0;
			if (Instigator != None)
				Direction = Location - Instigator.Location;
		...
		//damage - Damage should be applied constantly over DamageRadius
		if (Grenade != None && Distance <= DamageRadius)
		{
			//event Actor::
			//  TakeDamage(int Damage,  Pawn EventInstigator,   vector HitLocation, vector Momentum,    class<DamageType> DamageType    );
				TakeDamage(Damage,      Instigator,				GrenadeLocation,    vect(0,0,0),        class'Engine.GrenadeDamageType' );
		...
		if (Distance <= StingRadius)
		{
			...
			GetCommanderAction().NotifyStung(Grenade, GrenadeLocation, StungDuration);
		}
	}
}
...
simulated function SetCanBeArrested()
{
    bCanBeArrested = true;
}
...
simulated function bool CanBeArrestedNow() // TSR Change: Can be arrested if stunned, no longer checks if compliant
{
    return IsNonlethaled() && !IsArrested() && !IsBeingArrestedNow() && class.static.checkConscious(self);
}
...
defaultproperties
{
	OfficerMesh=Mesh'SWATMaleAnimation.SwatOfficer'
    bNoRepMesh=false
